...
class BaseDataset(Dataset):
    ...
    def __init__(self, dataroot: str,
        ...
        self.hr_downsampled_transform = transforms.Compose([
            transforms.RandomCrop((downFrom, downFrom)),
            transforms.Resize((image_size, image_size), IMode.BICUBIC),
            transforms.RandomHorizontalFlip(),
            transforms.RandomVerticalFlip(),
            transforms.ToTensor()
        ])
    ...
    def __getitem__(self, index):
        ...
        if self.downSize > 0 and self.downSize > self.imgSize:
            hr = self.hr_downsampled_transform(hr)
        else:
