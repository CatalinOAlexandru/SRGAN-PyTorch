...
parserTest.add_argument("--image-size", default=100, type=int, help="Image size of high resolution image. (Default: 96)")
...
def sr(model, hr_filename, sr_filename, cropSize):

    with torch.no_grad():
        img = cv2.imread(hr_filename) 
        imgTensor = torch.zeros((img.shape[2],img.shape[1],img.shape[0])).to(device)
        ...
        start = time.time()
        for i in range(0,img.shape[0],cropSize):
            for j in range(0,img.shape[1],cropSize):
                patches+=1
                newImg = img[i:i+cropSize,j:j+cropSize]
                # print('input image',newImg.shape)

                downDim = (int(newImg.shape[0]/args.scale_factor),int(newImg.shape[1]/args.scale_factor)) 
                downImg = cv2.resize(newImg,downDim,interpolation = cv2.INTER_CUBIC)
                downImg = cv2.cvtColor(downImg, cv2.COLOR_BGR2RGB)
                downImgTensor = ToTensor()(downImg).unsqueeze(0).to(device)
                # print('down img',downImg.shape)

                upImg = model(downImgTensor)
                # upImg = upImg[...,4:-4,4:-4]
                # upImg = upImg.permute(0,1,3,2)
                # print(upImg.shape)
...
def main():
    ...
    for index in range(len(filenames)):
        ...
        sr(model, hr_filename, sr_filename, args.image_size)
        ...
        logger.info(f"PSNR: {psnr:.2f} | SSIM: {ssim:.4f} | PerceptSim: {perpSimi:.4f}")
